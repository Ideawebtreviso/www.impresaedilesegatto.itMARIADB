...
function scaricobolla_leggiPrezzoEScontiDaDB(IDBOLLA) {
    // se non ho passato nessun parametro prendo IDBOLLA dalla label in pagina
    if (IDBOLLA == null)
...
function scaricobolla_eliminaRigaCosto() {
    var elQueryUpdate = document.getElementsByClassName("iwebSQLQUERY1Elimina")[0];
    if (elQueryUpdate != null) {
        var sqlQuery = iwebGeneraSqlQueryDaSpanSql(elQueryUpdate);
        var parametriQuery = iwebGeneraParametriQueryDaSpanSql(elQueryUpdate);
        ...
        xmlhttp.onreadystatechange = function () {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                ...
                if (jsonRisultatoQuery == "" || jsonRisultatoQuery[0].errore == null) {
                    // codice qui
                    iwebCaricaElemento("tabellaRigheScaricate", true, function () {
                        chiudiPopupType2B(document.getElementById("popupTabellaRigheScaricateElimina"));
                    });
