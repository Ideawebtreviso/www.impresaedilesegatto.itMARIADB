...
function get_mouseclick(event) {
    //if (window.event == null) event = arguments.callee.caller.arguments[0];
    iweb_EVENTO = window.event || event;
    ...
    if (el.className == "popup popupType1" || el.className == "popup popupType1 apriPopup")
        chiudiPopupType1(el);
}
            ...
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                ...
                if (jsonRisultatoQuery[0].errore == null) {
                    ...
                    if (nRigheContate == 0) {
                        ...
                        // 0 record trovati
                        elTabella.getElementsByTagName("tfoot")[0].getElementsByClassName("iwebTOTRECORD")[0].innerHTML = "Trovati 0 record";
                    } else {
...
function iwebTABELLA_impostaFiltri(idTabella) {
    applyToElements(document.getElementById(idTabella).getElementsByTagName("thead")[0].getElementsByClassName("iwebFILTRO"), function (el) {
        if (el.className.indexOf("iwebFILTRORICORDA_") >= 0) {
            var elInput = el.getElementsByTagName("input")[0];
            if (elInput != null) elInput.value = localStorage.getItem("iwebFILTRORICORDA_" + el.className.split("iwebFILTRORICORDA_")[1].split(" ")[0]);
        }
    });
}
...
function iwebTABELLA_VerificaAutocompletamento(el) {
    ...
    if (el.onkeyup != undefined && el.onkeyup != null) {
        ...
        if (stringa == undefined ||
            ...
            stringa == "") {
            stringa = "1000px";
            el.style.maxHeight = stringa;
        } else {
            stringa = (parseInt(stringa.replace("px", "")) + 1) + "px";
            el.style.maxHeight = stringa;
        }
        setTimeout(function () {
            var newStringa = el.style.maxHeight;
            if (newStringa == stringa) {
                // aggiorno il localstorage
                var elIwebFiltro = cercaElPadreConClasseRicors(el, "iwebFILTRO");
                if (elIwebFiltro.className.indexOf("iwebFILTRORICORDA_") >= 0)
                    localStorage.setItem("iwebFILTRORICORDA_" + elIwebFiltro.className.split("iwebFILTRORICORDA_")[1].split(" ")[0], el.value);
        ...
        var valore = el.value;
        setTimeout(function () {
            if (el.value.substr(0, el.value.length - 1) == valore) {
                // aggiorno il localstorage
                var elIwebFiltro = cercaElPadreConClasseRicors(el, "iwebFILTRO");
                if (elIwebFiltro.className.indexOf("iwebFILTRORICORDA_") >= 0)
                    localStorage.setItem("iwebFILTRORICORDA_" + elIwebFiltro.className.split("iwebFILTRORICORDA_")[1].split(" ")[0], el.value);
...
function iwebTABELLA_AssociaDatoInPopup(popupAssociato, nomeCampo, valoreRiga) {
    ...
    if (nomeNodo == "input") {
        ...
        // non è un elemento DDL che carica valori dal db, però è comunque una select alla quale aggiornare i valori
        if (elTemp.className.indexOf("iwebDDL") < 0) {
            // suppongo che valoreRiga contenga la chiave, quindi faccio l'associazione.
            var listaOption = elTemp.getElementsByTagName("option");
            for (var i = 0; i < listaOption.length; i++) {
                if (listaOption[i].value == valoreRiga) {
                    elTemp.selectedIndex = i;
                }
            }
        }
    }
...
function iwebTABELLA_EliminaRigaInPopup(IdPopupAssociato) {
    ...
    // apro il popup di elimina
    apriPopupType2_bind(IdPopupAssociato, true)

    // esegui l'eventuale funzione al termine del caricamento dell'evento
    iwebEseguiFunzioneAFineCaricamentoSePresente(popupAssociato.className);
}
...
function iwebTABELLA_ModificaRigaInPopup(IdPopupAssociato) {
    ...
    for (var i = 0; i < colonneDaModificare.length; ++i) {
        ...
        // BUTTA il codice commentato qua sotto
        /*var elTemp = popupAssociato.getElementsByClassName("iwebCAMPO_" + colonneDaModificare[i])[0];
        var nomeNodo = elTemp.nodeName.toLowerCase();
        if (nomeNodo == "input") {
            if (elTemp.type == "text")
...
function iwebTABELLA_ConfermaModificaRigaInPopup(IdPopupAssociato, idTabella, parametriSet, parametriWhere, attesaRispostaServer, funzioneAFineEsecuzione) {
    ...
    var popupAssociato = document.getElementById(IdPopupAssociato);
    if (popupAssociato == null)
        console.log("%cErrore in iwebTABELLA_ConfermaModificaRigaInPopup: non trovato l'elemento " + IdPopupAssociato, "color:darkred");
    ...
    if (elQueryUpdate != null) {
        ...
        if (campiObbligatoriVerificati.length == 0) {
            ...
            for (var i = 0; i < colonneDaModificare.length; ++i) {
                ...
                //iwebTABELLA_AssociaDatoInPopup(popupAssociato, nomeCampo, valoreRiga) -> esiste
                //iwebTABELLA_AssociaDatoDaPopup(popupAssociato, trPadre, nomeCampo, valoreRiga) -> dovrei creare
                ...
                if (classiNodo2.indexOf("iwebCodice") >= 0 || classiNodo2.indexOf("iwebTitolo") >= 0 || classiNodo2.indexOf("iwebDescrizione") >= 0) {
                    ...
                    if (valoreRiga != null && valoreRiga != "" && valoreRiga.length > caratteriDaMostrare) {
                        //elTemp.title = valoreRiga; -> HO CANCELLATO QUESTO PER SCRIVERE QUELLO SOTTO, credo sia giusto così
                        elTemp2.title = valoreRiga; // valore completo sul title nell'elemento che vado ad aggiornare
...
// pre-caricamento di un filtro
function iwebImpostaCampi(idElemento) {
    var elementoDaCaricare = document.getElementById(idElemento);
    if (elementoDaCaricare == null)
        console.log("'" + idElemento + "' non esiste, non posso caricarlo. (iwebImpostaCampi)");
    var nomeClasse = elementoDaCaricare.className;

    if (nomeClasse.indexOf("iwebTABELLA") >= 0) {
        iwebTABELLA_impostaFiltri(idElemento);
        console.log("precarico i filtri di iwebTABELLA " + idElemento);
    /*} else if (nomeClasse.indexOf("iwebDDL") >= 0) {
        iwebDDL_Carica(idElemento, attesaRispostaServer, funzioneAFineEsecuzione);
        console.log("carico iwebDDL " + idElemento);
    } else if (nomeClasse.indexOf("iwebLABEL") >= 0) {
        iwebLABEL_Carica(idElemento, funzioneAFineEsecuzione);
...
function iwebCaricaElemento(idElemento, attesaRispostaServer, funzioneAFineEsecuzione) {
    ...
    if (nomeClasse.indexOf("iwebTABELLA") >= 0) {
        ...
        iwebDETTAGLIO_Carica(idElemento, attesaRispostaServer, funzioneAFineEsecuzione);
...
// attesaRispostaServer non usato
function iwebDETTAGLIO_Carica(idElemento, attesaRispostaServer, funzioneAFineEsecuzione) {
    var el = document.getElementById(idElemento);
        ...
        xmlhttp.onreadystatechange = function () {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                ...
                if (jsonRisultatoQuery[0].errore == null) {
                    ...
                    // se non sono stati trovati elementi mostra Nessun elemento trovato e azzera il conteggio delle righe trovate
                    if (nRigheContate == 0) {
...
function iwebApriPopupModificaiwebDETTAGLIO(idElTableAnagrafica, idPopupAssociato) {
    ...
    if (elTableAnagrafica.getElementsByTagName("tbody")[1].getElementsByTagName("tr").length > 1) {
        ...
        // apro il popup di modifica dettaglio
...
function iwebGeneraParametriQueryDaSpanSql(elQuery) {
    ...
    for (var i = 0; i < nChiocciole; i++) {
        ...
        if (stringaParametro.indexOf("=") == -1)
            console.log("%cErrore nella valutazione del parametro: " + stringaParametro + ". Il parametro dovrebbe avere una sintassi come: @nomeParametro = valoreParametro", "color:darkred");

        // se trovo !!! devo loggare questo parametro
        var logParametro = false;
        if (stringaParametro.search("!!!") >= 0) {
            logParametro = true;
            stringaParametro = stringaParametro.replace("!!!", ""); // tolgo la stringa di log
        }
        ...
        // log del parametro
        if (logParametro)
            console.log("Parametro:" + stringaParametro + "   Valore:" + valoreParametro);
    ...
    if (leftString(stringa, "VALORE_".length).toUpperCase() == "VALORE_") {
        ...
        if (el.className.indexOf("iwebTABELLA") >= 0 && azione.toLowerCase() == "findfirstvalue") {
            ...
            if (elSelezionato != null && elSelezionato.getElementsByClassName("iwebCAMPO_" + nomeCampo).length > 0) {
                ...
                var nomeNodo = campoSelezionato.nodeName.toLowerCase();

                // fino ad ora non mi sono capitati casi diversi da span ad eccezione di input[type=checkbox]
                if (nomeNodo == "input" && campoSelezionato.type == "checkbox")
                    risultatoValuzione = campoSelezionato.checked;
                else
...
function cercaElPadreConClasseRicors(el, nomeClasse) {
    if (el.nodeName.toLowerCase() == "body") return el; // in caso di body, restituisco il body
        ...
        if (elementoContieneClasse(el, nomeClasse) == true) return el;
        else el = cercaElPadreConClasseRicors(el.parentElement, nomeClasse);
    }
    return el;
}
// cerco il primo elemento padre che ha il tag specificato
function cercaElPadreConTagRicors(el, nomeTAG) {
    if (el.nodeName.toLowerCase() == "body") return el; // in caso di body, restituisco il body
    else {
        if (el.tagName.toLowerCase() == nomeTAG.toLowerCase()) return el;
        else el = cercaElPadreConTagRicors(el.parentElement, nomeTAG);
    }
    return el;
}
...
function iwebLABEL_Carica(idElemento, funzioneAFineEsecuzione) {
    var el = document.getElementById(idElemento);
...
function iwebEseguiFunzioneAFineCaricamentoSePresente(nomeClasse) {
    if (nomeClasse.indexOf("iwebfunzione_") >= 0) {
        // prendo ciò che sta a destra di "iwebfunzione_" e a sinistra di " "
        // es: nomeClasse = "popup popupType2 iwebBIND__idelemento__idelement2 iwebfunzione_funzionePopupTabellaBolleElimina" -> "funzionePopupTabellaBolleElimina"
        var nomeFunzione = nomeClasse.split("iwebfunzione_")[1].split(" ")[0];
        // esegui la funzione
        console.log("%cNota: eseguo la funzione " + nomeFunzione + "()", "color:green");
        eval(nomeFunzione + "()");
    }
}

function iwebEseguiQuery(idElQuery, attesaRispostaServer, funzioneAFineEsecuzione) {
    var elQueryUpdate = document.getElementById(idElQuery);
