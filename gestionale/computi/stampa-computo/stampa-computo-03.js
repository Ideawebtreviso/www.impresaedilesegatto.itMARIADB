function stampaComputo() {
    ...
    salvaDatiComputo();
    //salvaPDFSuddivisioniEAggiornaComputo();
    //lanciaStampa();
}
            ...
            xmlhttp.onreadystatechange = function () {
                if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                    ...
                    if (jsonRisultatoQuery == "" || jsonRisultatoQuery[0].errore == null) {
                        ...
                        salvaPDFSuddivisioniEAggiornaComputo(idStampa);
...
function salvaPDFSuddivisioniEAggiornaComputo(idStampa) {
    ...
    var suddivisioni = [];
    var idcomputo = iwebValutaParametroAjax("IDCOMPUTO_value");
    var condizioniultimapagina = iwebValutaParametroAjax("condizioniUltimaPagina_value");
    ...
    xmlhttp.open("POST", getRootPath() + "/WebServiceComputi.asmx/salvaPDFSuddivisioniEAggiornaComputo", true);
    var jsonAsObject = {
        idStampa: idStampa,
...
function ristampaComputoPrecedentementeGenerato() {
    var idStampa = iwebValutaParametroAjax("tabellaStampe_selectedValue_idcomputopdf");
    ...
    xmlhttp.open("POST", getRootPath() + "/WebServiceComputi.asmx/duplicaDatiPDFSuddivisioniEComputo", true);
    var jsonAsObject = {
        idcomputopdf: parseInt(idStampa)
    }
    xmlhttp.setRequestHeader("Content-type", "application/json");
    var jsonAsString = JSON.stringify(jsonAsObject);
    xmlhttp.send(jsonAsString);
