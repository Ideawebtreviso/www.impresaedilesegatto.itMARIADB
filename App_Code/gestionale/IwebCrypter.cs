...
/* htt p://w ww.superstarcoders. com/blogs/posts/symmetric-encryption-in-c-sharp.aspx
...
public class IwebCrypter
{
    ...
    // CipherUtility.iwebcsCriptaSQL("SELECT * FROM utenti WHERE id = 4") oppure CipherUtility.iwebcsDecifraSQL(*stringa crittografata*)
    public static string iwebcsCriptaSQL(string sql)
    {
        //string encrypted = sql;
        string encrypted = Encrypt<AesManaged>(sql, "password", "salt");
        return encrypted;
    }

    public static string iwebcsDecifraSQL(string sqlCriptato)
    {
        //string decrypted = sqlCriptato.Replace("&lt;", "<").Replace("&gt;", ">");
        string decrypted = Decrypt<AesManaged>(sqlCriptato, "password", "salt");
        return decrypted;
    }
